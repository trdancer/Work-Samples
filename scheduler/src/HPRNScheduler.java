...
public class HPRNScheduler extends Scheduler {
	LinkedList<Process> ready_queue;
	int counter = 1;
	public HPRNScheduler(List<Process> processes, boolean v, String fn) {
		super(processes, v);
		try {
			writer = new BufferedWriter(new FileWriter(".."+sep+"out"+sep+"hprn-output-"+fn+type));
		}
		catch (IOException e) {
	...
	public void execute() {
		// System.out.println("Executing HPRN.");
		// for (Process q : original_processes) {
		// 	q.print();
		// 	//if running process is null, choose next from ready queue
		// }
		while (true) {
			running_process = getHighestPenaltyRatio();
		...
		write_file(writer, "The scheduling alogrithm used was Highest Penalty Ratio Next\n\n");
	...
	public void updateBlocked() {
		// Collections.<Process>sort(blocked, Collections.reverseOrder());
		if (blocked.size() != 0) {
			total_io_cycles++;
		}
	...
	Process getHighestPenaltyRatio() {
		if (ready_queue.size() == 0) {
			return null;
		}
		Process max = ready_queue.getFirst();
		double temp = 0.0;
		for (Process p : ready_queue) {
			double r = getPenaltyRatio(p);
			// System.out.println("Process "+p.id+" ratio is: "+r);
			if (r > temp) {
				max = p;
				temp = r;
			}
			else if (r == temp) {
				switch (max.compareTo(p)) {
					case -1:
						break;
					case 0:
						break;
		...
		ready_queue.remove(max);
		// System.out.println("I chose to run process "+max.id+" with a the highest ratio of "+temp);
		return max;
	}
	double getPenaltyRatio(Process p) {
		// T = time this process has been in the system
		//t = time this process has been running
		//ratio = T/t
		double ratio = (double) (cycles_elapsed-p.arrival_time)/(double) p.get_running_time();
		return ratio;
	}
}