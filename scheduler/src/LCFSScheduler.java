...
public class LCFSScheduler extends Scheduler {
	...
	int counter = 1;
	public LCFSScheduler(List<Process> processes, boolean v, String fn) {
		super(processes, v);
		try {
			writer = new BufferedWriter(new FileWriter(".."+sep+"out"+sep+"lcfs-output-"+fn+type));
		}
		catch (IOException e) {
	...
	public void execute() {
		// System.out.println("Executing LCFS.");
		// for (Process p : original_processes) {
		// 	p.print();
		// }
		//if running process is null, choose next from ready queue
		while (true) {
			running_process = ready_queue.pollFirst();
		...
		write_file(writer, "The scheduling alogrithm used was Last Come First Served\n\n");
